This is all based on the initial algebra semantics for GADTs
\cite{Johann:2007}.
...
% subsection the_non_graded_case (end)
...
\subsubsection{Graded Folds and Fold Fusion}
...
$\mathsf{fold}(h')$.  
% subsubsection graded_folds (end)
...
\end{lemma}
\begin{proof}
    Suppose $(\func{Y},v')$ is another \ULimit{F}. Thus, there is a
    unique \UCone{F} morphism $i : \func{Y} \mto \func{X}$ such that
    $i;v = v'$. But, there must also be a unique \UCone{F} morphism $j
...
% 
\noindent
We denote a chosen \ULimit{K} by
$(\mu^*\func{K},\mathsf{fold}^*_\func{K})$ and
$\mathsf{build}^*_\func{K}$ for the mediating map from some other
\UCone{K} $(\func{X},v)$.

Every graded initial $\func{K}$-algebra is also a \ULimit{F}.
% 
\begin{lemma}[Initial Algebras are also Limits]
...
\cite{Ghani:2004}.
\end{proof}
% subsubsection graded_fold_build_fusion_rules (end)

\todoi{If there is time, add the dinaturality from \cite{Ghani:2004}}

% subsection initial_algebra_packages_for_basic_ggadts (end)